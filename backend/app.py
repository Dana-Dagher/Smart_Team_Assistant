...
#     # Apply filters
#     if status_filter and status_filter != 'All': # 'All' will be a frontend option
#         query = query.filter_by(status=status_filter)

#     if assigned_to_id_filter:
#         # Handle 'Unassigned' or specific user
#         if assigned_to_id_filter == -1: # Frontend will send -1 for Unassigned
...
#     # Add more sorting options as needed (e.g., 'status', 'id')
...
#     # E.g., if no valid sort_by, then sort by ID
...
# @app.route('/tasks', methods=['GET', 'POST']) # Added 'POST' method
# def tasks():
...
#                 "status": task.status,
#                 "deadline": task.deadline,
#                 "assigned_to": assignee_name, # Display name, not ID
#                 "assigned_to_id": task.assigned_to_id, # Keep ID for internal use
#                 "assignment_explanation": task.assignment_explanation
#             })
...
#             db.session.commit()
#             # Return the newly created task's data, including its generated ID
